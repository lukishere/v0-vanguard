...
function normalizeDemoAccess(access: unknown): ClientDemoAccess[] {
...
function normalizeCustomContent(
  content: unknown
...
export function getClientMetadataFromUser(
  user: User | null
): ClientPublicMetadata {
  ...
  const lastActivity =
    typeof metadata.lastActivity === "string"
    ...
    await client.users.updateUser(userId, {
      publicMetadata: {
        ...
        ...(merged.customContent
          ? { customContent: merged.customContent }
          : {}),
      },
    });
...
export async function upsertClientDemoAccess(
  ...
  const existingIndex = current.demoAccess.findIndex(
    (a) => a.demoId === newAccess.demoId
  );
