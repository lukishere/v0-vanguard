...
export default async function ClientDashboardPage() {
  ...
  if (!userId) {
    console.error(
      "🔒 [Dashboard] No authenticated user found, this should be handled by middleware"
    );
  ...
  console.log(
    "🎭 [Dashboard] Rol detectado:",
    userIsAdmin ? "ADMIN" : "CLIENTE"
  );
  ...
  const clientAccess = Array.isArray(clientMetadata.demoAccess)
    ? clientMetadata.demoAccess
    : [];
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(
      `📊 [Dashboard] ${activeDemos.length} demos activas, ${clientAccess.length} accesos totales`
    );
  }
  ...
  const expiringDemos = activeDemos.filter(
    (demo) =>
