...
async function getStorageUser() {
  ...
  try {
    ...
    console.log(
      "⚠️ [Demo Requests] Usando almacenamiento distribuido por usuario"
    );
    return null;
  } catch (error) {
    console.error(
      "❌ [Demo Requests] Error obteniendo usuario de almacenamiento:",
      error
    );
    return null;
  }
...
Equipo de Soporte`;
    ...
    if (!messageResult.success) {
      console.warn(
        "⚠️ No se pudo enviar el mensaje de notificación al cliente:",
        messageResult.error
      );
    }
